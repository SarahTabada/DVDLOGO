def tick args
  ...
  args.state.boundaries ||= [
    { target: {x: 0, y: 0, w: 5, h: 720}, growth_direction: {x: 0, y: 0, w: 1, h: 0}, min_size: {w:5, h: 720}, bouncedir: {x: 5, y: 0}, x: 0, y: 0, w: 5, h: 720, path: :solid, r: 100, g: 149, b: 237, maxclicks:65 },
    { target: {x: 1275, y: 0, w: 5, h: 720}, growth_direction: {x: -1, y: 0, w: 1, h: 0}, min_size: {w:5, h: 720}, bouncedir: {x: -5, y: 0}, x: 1275, y: 0, w: 5, h: 720, path: :solid, r: 100, g: 149, b: 237, maxclicks:65 },
    { target: {x: 0, y: 0, w: 1280, h: 5}, growth_direction: {x: 0, y: 0, w: 0, h: 1}, min_size: {w:1280, h: 5}, bouncedir: {x: 0, y: 5}, x: 0, y: 0, w: 1280, h: 5, path: :solid, r: 141, g: 79, b: 58, maxclicks:100000 },
    { target: {x: 0, y: 715, w: 1280, h: 5}, growth_direction: {x: 0, y: -1, w: 0, h: 1}, min_size: {w:1280, h: 5}, bouncedir: {x: 0, y: -5}, x: 0, y: 715, w: 1280, h: 5, path: :solid, r: 100, g: 149, b: 237, maxclicks:100000 },
  ]
  ...
  args.state.boundaries.each do |boundary|
    ...
    if boundary.bouncedir[:x] == 5 
      # Stop when right edge touch block's left  edge
      should_shrink = (boundary.target.x + boundary.target.w + boundary.growth_direction.w) < block.x
    elsif boundary.bouncedir[:x] == -5    
      # Stop when left edge touch block's right edge
      should_shrink = (boundary.target.x + boundary.growth_direction.x) > (block.x + block.w) - 2
    elsif boundary.bouncedir[:y] == 5  
      # Stop when top edge touch block's bottom edge  
      should_shrink = (boundary.target.y + boundary.target.h + boundary.growth_direction.h) < block.y 
    elsif boundary.bouncedir[:y] == -5 
      # Stop when bottom  edge touch block's top edge
      should_shrink = (boundary.target.y + boundary.growth_direction.y) > (block.y + block.h) + 10
    end

    # only shrink if haven't reached the block and above minimum size
    if should_shrink && 
